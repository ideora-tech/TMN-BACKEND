...
<head>
    <meta charset="utf-8">
    <title>Riwayat Pemegang Armada</title>
    <style>
        @page { margin: 0; }
    ...
    <div class="konten">
        ...
        <table class="rincian">
            ...
            <tbody>
                ...
                <tr @if($item->dihapus_pada !== null) style="color:#9ca3af" @endif>
                    <td>{{ date('d/m/Y', strtotime($item->tanggal)) }}</td>
                    <td><strong>{{ $item->supir_nama }}</strong></td>
                    <td>{{ $item->nama_proyek ?? '-' }}</td>
                    <td>{{ $labelSumber[$item->sumber] ?? $item->sumber }}</td>
                    <td>{{ $item->pemilik_nama ?? '-' }}</td>
                    <td>{{ $item->keterangan ?? '-' }}</td>
                    <td>{{ $item->dihapus_pada === null ? 'Berlaku' : 'Digantikan ' . date('d/m H:i', strtotime($item->dihapus_pada)) }}</td>
                </tr>
                @empty
                <tr><td colspan="7" class="ket" style="text-align:center">Belum ada riwayat alokasi pada periode ini</td></tr>
                @endforelse
            </tbody>
        </table>
