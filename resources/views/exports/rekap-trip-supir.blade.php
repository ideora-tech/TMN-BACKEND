...
<body>
    @php
        ...
        $totalTrip = collect($items)->sum(fn ($i) => (int) $i->jumlah_trip);
        $totalSelesai = collect($items)->sum(fn ($i) => (int) $i->selesai);
        $totalBatal = collect($items)->sum(fn ($i) => (int) $i->dibatalkan);
        $totalJarak = collect($items)->sum(fn ($i) => (float) ($i->total_jarak_km ?? 0));
        ...
        <table class="rincian">
            <thead>
                <tr>
                    <th>NAMA SUPIR</th>
                    <th>SUMBER</th>
                    <th class="jumlah">JUMLAH TRIP</th>
                    <th class="jumlah">SELESAI</th>
                    <th class="jumlah">DIBATALKAN</th>
                    <th class="jumlah">TOTAL JARAK (KM)</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="jumlah">{{ (int) $item->jumlah_trip }}</td>
                    <td class="jumlah">{{ (int) $item->selesai }}</td>
                    <td class="jumlah">{{ (int) $item->dibatalkan }}</td>
                    <td class="jumlah">{{ number_format($item->total_jarak_km ?? 0, 2, ',', '.') }}</td>
                ...
                @empty
                <tr><td colspan="8" class="ket" style="text-align:center">Tidak ada data trip pada periode ini</td></tr>
                @endforelse
                @if(count($items) > 0)
                <tr class="subtotal">
                    <td colspan="2">TOTAL</td>
                    <td class="jumlah">{{ $totalTrip }}</td>
                    <td class="jumlah">{{ $totalSelesai }}</td>
                    <td class="jumlah">{{ $totalBatal }}</td>
