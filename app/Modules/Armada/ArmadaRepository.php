...
class ArmadaRepository implements ArmadaRepositoryInterface
{
    ...
    /**
     * Warning servis berbasis KILOMETER: per armada × paket interval_km jenis
     * kendaraannya, hitung sisa km = (km servis terakhir paket itu + interval_km)
    ...
    public function findServisJatuhTempoKm(string $idPerusahaan): array
    {
        $rules = DB::table('interval_perawatan as ip')
            ...
            ->get(['ip.id_interval_perawatan', 'ip.id_jenis_kendaraan', 'ip.interval_km', 'ip.interval_bulan']);
        ...
        $servisTerakhir = DB::table('perawatan_armada as p1')
            ...
            ->get(['p1.id_armada', 'p1.id_interval_perawatan', 'p1.km_odometer']);

        $servisMap = [];
        foreach ($servisTerakhir as $row) {
            $servisMap[$row->id_armada . '|' . $row->id_interval_perawatan] = (int) $row->km_odometer;
        }
        ...
        foreach ($armadaList as $armada) {
            ...
            foreach ($rulesPerKendaraan->get($armada->id_jenis_kendaraan, collect()) as $rule) {
                $kmServis = $servisMap[$armada->id_armada . '|' . $rule->id_interval_perawatan] ?? null;
                if ($kmServis === null) {
                    continue;
                }
                ...
                $hasil[] = [
                    ...
                    'jenis_perawatan' => IntervalLabelBuilder::buildOrFallback(
                        $intervalKm,
                        $rule->interval_bulan !== null ? (int) $rule->interval_bulan : null,
                    ),
